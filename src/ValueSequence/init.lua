...
function ValueSequence:Lerp(other: ValueSequence, alpha: number)
	...
	for t, v in pairs(ts) do
		-- print("T", t, "V", v)
		local val = Algebra.lerp(v.Value[1], v.Value[2], alpha)
		local min = Algebra.lerp(v.Min[1], v.Min[2], alpha)
		local max = Algebra.lerp(v.Max[1], v.Max[2], alpha)
	...
	return ValueSequence.new(keypoints, Algebra.lerp(self.Seed, other.Seed, alpha))
end

function ValueSequence:GetValue(
	alpha: number,
	easingStyle: Enum.EasingStyle | string | nil,
    easingDirection: Enum.EasingDirection | string | nil,
	...
	for i = 1, #self.Keypoints - 1 do
		...
		if easedAlpha >= ths.Alpha and easedAlpha < nxt.Alpha then
			...
			if minMaxAlpha then
				local min = Algebra.lerp(ths.Min, nxt.Min, nxtAlpha)
				local max = Algebra.lerp(ths.Max, nxt.Max, nxtAlpha)

				return Algebra.lerp(min, max, minMaxAlpha)
			else
				return Algebra.lerp(ths.Value, nxt.Value, nxtAlpha)
			end
